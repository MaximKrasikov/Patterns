...
public class Solution {
    ...
    public static void main(String[] args) {
        Storage storage = new FarAwayStorage();//объект класса FarAwayStorage

        fillStorage(storage);

        System.out.println("Testing OriginalRetriever: ");
        testRetriever(new OriginalRetriever(storage));// загрузка из storage

        System.out.println("\n\n\n\n\n\n\n\n\n\nTesting CachingProxyRetriever: ");
        testRetriever(new CachingProxyRetriever(storage));// загрузка через кэш
    }
